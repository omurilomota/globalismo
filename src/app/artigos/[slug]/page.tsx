...
import StructuredData, { generateArticleData, generateBreadcrumbData } from '@/components/seo/StructuredData';
...
export default async function ArtigoPage({ params }: PageProps) {
  ...
  const breadcrumbData = generateBreadcrumbData([
    { name: 'Home', url: 'https://globalismo.com.br' },
    { name: 'Artigos', url: 'https://globalismo.com.br/artigos' },
    { name: article.titulo, url: getCanonicalUrl(`/artigos/${article.slug}`) }
  ]);

  return (
    <>
      <StructuredData data={generateArticleData(article)} />
      <StructuredData data={breadcrumbData} />
      <article className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <Breadcrumbs items={[
          { name: 'Artigos', href: '/artigos' },
          { name: article.titulo, href: `/artigos/${article.slug}` }
        ]} />
        
        <header className="mb-8">
          <div className="flex flex-wrap gap-2 mb-4" role="list" aria-label="Categorias do artigo">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 text-gray-500 dark:text-gray-400 text-sm mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {relatedArticles.map((related) => (
                <Link
                  ...
                  className="block bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-5 hover:shadow-md transition-shadow focus:outline-none focus:ring-2 focus:ring-blue-900 dark:focus:ring-blue-400 rounded"
                >
        ...
        <div className="mt-12">
          <Link 
            href="/artigos"
            className="inline-flex items-center text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-blue-900 dark:focus:ring-blue-400 rounded px-2 py-1 -ml-2"
          >
